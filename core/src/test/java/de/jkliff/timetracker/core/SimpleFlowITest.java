...
public class SimpleFlowITest {
    ...
    public void testSimpleFlow ()
        ...
        try {
            ...
            Activity a1 = ActivityBuilder.forGroup ("default").startedAt ("2011-10-10 13:00").finishedAt ("2011-10-10 14:00")
                    .named (A1_NAME).withTags ("test", "study", "music").build ();
            ...
            List<ActivitySummary> activitySummaries = activityService.list (QueryBuilder.findActivitySummaries ().taggedWith ("work")
                    .build ());
            Assert.assertNotNull (activitySummaries);

            activitySummaries = activityService.list (QueryBuilder.findActivitySummaries ().startedBefore ("2011-10-10 09:00")
                    .endedAfter ("2011-10-10 15:00").taggedWith ("work").build ());

            Assert.assertNotNull (activitySummaries);
            Assert.assertEquals (0, activitySummaries.size ());

            activitySummaries = activityService.list (QueryBuilder.findActivitySummaries ().startedAfter ("2011-10-10 13:00")
                    .endedBefore ("2011-10-10 16:30").taggedWith ("work").build ());
            Assert.assertNotNull (activitySummaries);
            Assert.assertEquals (6, activitySummaries.size ());

            activitySummaries = activityService.list (QueryBuilder.findActivitySummaries ().longerThan ("1d").taggedWith ("work").build ());
            Assert.assertNotNull (activitySummaries);
            Assert.assertEquals (9, activitySummaries.size ());

            activitySummaries = activityService.list (QueryBuilder.findActivitySummaries ().shorterThan ("30 min").taggedWith ("work")
                    .build ());
            Assert.assertNotNull (activitySummaries);
            Assert.assertEquals (9, activitySummaries.size ());
    ...
    public void saveTestData ()
        ...
        Activity[] activities = new Activity[] {
                ActivityBuilder.forGroup ("default").startedAt ("2011-10-10 13:00").finishedAt ("2011-10-10 13:30").named ("tomando banho")
                        .withTags ("test").build (),
                ...
                ActivityBuilder.forGroup ("default").startedAt ("2011-10-10 14:30").finishedAt ("2011-10-10 15:00").named ("almoço")
                        .withTags ("test").build (),
                ...
                ActivityBuilder.forGroup ("default").startedAt ("2011-10-12 09:00").finishedAt ("2011-10-12 14:00").named ("estudando")
                        .withTags ("teste").build (),
